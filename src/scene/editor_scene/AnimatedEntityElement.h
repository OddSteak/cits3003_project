...
class AnimatedEntityElement : virtual public SceneElement,
                              public LocalTransformComponent,
                              public LitMaterialComponent,
                              public AnimationComponent {
public:
  /// NOTE: Must be unique per element type, as it is used to select generators,
  ///       so if you are creating a new element type make sure to change this
  ...
  CustomMotionParams motion_parameters{};

  AnimatedEntityElement(
      const ElementRef &parent, std::string name, const glm::vec3 &position,
      const glm::vec3 &euler_rotation, const glm::vec3 &scale,
      std::shared_ptr<AnimatedEntityRenderer::Entity> rendered_entity)
      : SceneElement(parent, std::move(name)),
        LocalTransformComponent(position, euler_rotation, scale),
        LitMaterialComponent(rendered_entity->instance_data.material),
        AnimationComponent(), rendered_entity(std::move(rendered_entity)) {}

  static std::unique_ptr<AnimatedEntityElement>
  new_default(const SceneContext &scene_context, ElementRef parent);
  static std::unique_ptr<AnimatedEntityElement>
  from_json(const SceneContext &scene_context, ElementRef parent,
            const json &j);
  [[nodiscard]] json into_json() const override;

  void add_imgui_edit_section(MasterRenderScene &render_scene,
                              const SceneContext &scene_context) override;
