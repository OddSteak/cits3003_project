...
class Animator {
  std::unordered_map<std::shared_ptr<AnimatedEntityInterface>,
                     AnimationParameters>
      animated_entities{};
...
public:
  ...
  /// parameters.
  template <class AnimatedEntity>
  std::optional<AnimationParameters>
...
template <class AnimatedEntity>
void Animator::start(std::shared_ptr<AnimatedEntity> animated_entity,
                     AnimationParameters animation_parameters) {
...
template <class AnimatedEntity>
void Animator::update_param(std::shared_ptr<AnimatedEntity> animated_entity,
                            AnimationParameters animation_parameters) {
  auto aei =
      std::dynamic_pointer_cast<AnimatedEntityInterface>(animated_entity);
...
template <class AnimatedEntity>
void Animator::resume(std::shared_ptr<AnimatedEntity> animated_entity,
                      AnimationParameters animation_parameters) {
  auto aei =
      std::dynamic_pointer_cast<AnimatedEntityInterface>(animated_entity);
  auto param = animated_entities.find(aei);
...
template <class AnimatedEntity>
std::optional<AnimationParameters>
Animator::is_animating(const std::shared_ptr<AnimatedEntity> &animated_entity) {
  auto aei =
      std::dynamic_pointer_cast<AnimatedEntityInterface>(animated_entity);
  auto param = animated_entities.find(aei);
  if (param != animated_entities.end()) {
